#!/usr/bin/env python3
"""Owner-only local LLM gateway with bounded public-source enrichment.

This service binds to loopback by default and is intended to sit behind a
Cloudflare quick/named tunnel.  The public tunnel never reaches llama.cpp
directly: every request must carry II_LOCAL_LLM_SHARED_SECRET, and this gateway
forwards only to a loopback llama.cpp OpenAI-compatible endpoint.

For explicit stock symbols, the gateway builds a bounded public research context
from independent source families before calling the local model.  Source
failures degrade independently; they are never silently converted into facts.
"""
...
PORT = 8814
MAX_BODY = 256_000
TICKER_RE = re.compile(r"(?:^|[^A-Za-z0-9])\$?([A-Z][A-Z0-9]{0,5}(?:[.-][A-Z0-9]{1,4})?)(?=$|[^A-Za-z0-9])")
IGNORED = {
    "AI", "LINE", "BOT", "TOP", "CALL", "PUT", "SELL", "BUY", "OPTION",
    "OPTIONS", "BID", "ASK", "DTE", "IV", "YES", "NO", "USD", "CAGR",
}
CACHE_ROOT = Path(os.getenv("LOCALAPPDATA", str(Path.home()))) / "InvestorIntelligence" / "UserData" / "v212-local-llm-cache"
...
def extract_ticker(text: str) -> str | None:
    for match in TICKER_RE.finditer(text.normalize("NFKC") if hasattr(text, "normalize") else text):
        value = str(match.group(1) or "").upper()
        if value and value not in IGNORED:
            return value
    # Python str has no JS-style normalize method; retry on the original text.
...
def collect_sec(ticker: str) -> SourceResult:
    ...
    try:
        ...
        response = requests.get(url, headers=sec_headers(), timeout=(10, 45))
        response.raise_for_status()
...
def resolve_yahoo_symbol(ticker: str) -> str:
    ...
    try:
        ...
        if isinstance(quotes, list):
            ...
            equity = [
                item for item in quotes
                if isinstance(item, dict) and str(item.get("quoteType") or "").upper() in {"EQUITY", "STOCK"}
            ]
            if equity:
...
def collect_yahoo(ticker: str) -> SourceResult:
    retrieved = now_iso()
    if yf is None:
...
def collect_bls() -> SourceResult:
    ...
    try:
        response = requests.post(
            ...
            json={"seriesid": ["CUUR0000SA0", "LNS14000000"], "latest": True},
            timeout=(10, 30),
        )
        response.raise_for_status()
        payload = response.json()
...
def collect_world_bank() -> SourceResult:
    retrieved = now_iso()
    try:
        url = "https://api.worldbank.org/v2/country/USA/indicator/NY.GDP.MKTP.KD.ZG"
        response = requests.get(url, params={"format": "json", "per_page": 5}, timeout=(10, 30))
        response.raise_for_status()
        payload = response.json()
...
def build_source_context(ticker: str | None) -> dict[str, Any]:
...
def enrich_messages(messages: list[dict[str, Any]]) -> tuple[list[dict[str, str]], dict[str, Any]]:
    ...
    context = build_source_context(ticker)
    context_text = compact(context, 22_000)
    supplement = (
        "\n\n[V2.1.2 ON-DEMAND PUBLIC SOURCE ENSEMBLE]\n"
        + context_text
        + "\nRules for this block: source failures are not facts; distinguish official/regulated sources from T3 market observations; "
          "if source_diversity_status is DEGRADED, say coverage is limited; do not invent missing prices, filings, options or citations. "
...
class GatewayHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path != "/health":
            ...
            return
        try:
            response = requests.get(llama_url().replace("/v1/chat/completions", "/health"), timeout=(2, 5))
...
def main() -> int:
    ...
    if args.self_test:
        assert extract_ticker("AAOI 評分") == "AAOI"
        assert extract_ticker("SIVE呢") == "SIVE"
