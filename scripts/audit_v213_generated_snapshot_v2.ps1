...
function Ticker-At([object[]]$Rows,[int]$Index,[string]$Label) {
    Require ($Index -ge 0 -and $Index -lt $Rows.Count) "$Label index is out of range."
    $row = $Rows[$Index]
    $ticker = ([string](Get-Value $row 'ticker' '')).ToUpperInvariant()
    Require ($ticker -match '^[A-Z0-9][A-Z0-9.-]{0,14}$') "$Label contains an invalid ticker at rank $($Index + 1)."
    Require ((Integer (Get-Value $row 'rank' 0)) -eq ($Index + 1)) "$Label rank contract failed for $ticker."
    return $ticker
}
...
foreach ($name in $payloadNames) {
    $body = [string](Get-Value $payloads $name '')
    $expected = [string](Get-Value $digests $name '')
    Require (-not [string]::IsNullOrWhiteSpace($body)) "Activation payload missing: $name"
    Require ($expected -match '^[0-9a-f]{64}$') "Activation digest missing/invalid: $name"
    Require ((Sha256 $body) -eq $expected) "Activation payload digest mismatch: $name"
}
...
$v212Rows = @(Get-Value $v212 'records' @())
$v213Rows = @(Get-Value $v213 'records' @())
$fedRows = @(Get-Value $federation 'ticker_sources' @())
$auditRows = @(Get-Value $audit 'records' @())
Require ($top20.Count -eq 20) "Top20 count must be 20; found $($top20.Count)."
Require ($v212Rows.Count -eq 20) 'v2.1.2 report count must be 20.'
Require ($v213Rows.Count -eq 20) 'v2.1.3 report count must be 20.'
Require ($fedRows.Count -eq 20) 'Source federation count must be 20.'
Require ($auditRows.Count -eq 20) 'Source audit count must be 20.'
...
Require ([string](Get-Value $freshRoot 'policy_id' '') -eq [string](Get-Value $policy 'policy_id' '')) 'Freshness audit policy ID mismatch.'
...
for ($index = 0; $index -lt 20; $index++) {
    $top = $top20[$index]
    $ticker = Ticker-At $top20 $index 'Top20'
    Require ((Ticker-At $v212Rows $index 'v2.1.2 report') -eq $ticker) "$ticker order differs in v2.1.2 report."
    Require ((Ticker-At $v213Rows $index 'v2.1.3 report') -eq $ticker) "$ticker order differs in v2.1.3 report."
    Require ((Ticker-At $fedRows $index 'source federation') -eq $ticker) "$ticker order differs in source federation."
    Require ((Ticker-At $auditRows $index 'source audit') -eq $ticker) "$ticker order differs in source audit."
    ...
    if ($providerCount -lt 1) {
        ...
        $logicState = Get-Value $sourceRow 'public_logic_state' $null
        Require ([string](Get-Value $logicState 'model_inference_confidence' '') -eq 'LIMITED') "$ticker uncorroborated model confidence is not LIMITED."
...
$result = [ordered]@{
    ...
    ticker_count = 20
    all_payload_digests_verified = $true
...
Write-Host "V213_GENERATED_SNAPSHOT_AUDIT_V2 = PASS; run_id=$($result.run_id); tickers=20; payloads=7; digests=true; same_order=true; per_ticker_multi_source=true; positive_advantages_fresh=true; stale_market=0; conflicts=0" -ForegroundColor Green
