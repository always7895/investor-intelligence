...
### TASK0 Operability Audit — Phase 1C (2026-09-17T11:37–12:35Z; Pro-supervised)
- Base: audited ab1bcca; worktree clean at b3b4293; this-round whitelist: tests/test_v213_market_products.py + state/STATUS.md.
- **P1 FIXED**: test_cli_builder_synthetic_fixture subprocess pinned encoding="utf-8" (parent cp950 + child UTF-8 0xE7 repro @11:51Z; old strict decode raised, new decode holds all JSON assertions). Regression: standalone + file 12/12 + CP950 record script + FULL pytest **1521/0/3** @11:56:05Z.
- op-lock subtests: standalone -vv PASS @11:37:53Z; full-suite x2 (11:38–11:48Z) showed ONLY the market failure -> NOT REPRODUCED -> **P2 un-reproduced** (zero changes).
- **NEW P2**: fixed RUN 092410Z passed the 7200s prod-Top20 cap ~11:24:10Z -> v213-live-production-probe now single-F (freshness artifact, NOT a product fault): fresh runs 105616Z/115616Z landed; refresh log pointer-last 115616Z-bc4884011c22 seal a7e4c911… @11:56:16Z, 15/15, GEV 96.0/6501 93.0. Recommend fresh-or-expiry-pinned fixture next round.
- MODEL/RELAY: tabbyAPI :5000 (pid 5604) alive + live oneshot PASS; ABSENT: local bridge / relay-lease producer (no 8080/8376/8377, no app profile) -> free-relay DO lease (TTL 300s) local producer unverified = general-QA suspect strongest (unverified; no CF route read). V213_MODEL_PROFILE_JSON is CF-side -> live readiness-with-profile **BLOCKED_LOCAL_SOURCE_MISSING (UNKNOWN)**.
- Gates RC=0: security, documentation, workflow-supply-chain, canonical-RC-v2, pipeline-boundary self-test (local/separated: no publish/nonce/LINE/activation). vitest 817/1F (F above); tsc clean; pytest 1521/0/3. clean-install: **BLOCKED_SCOPE** not run (global scheduler/service conditions unmet).
- OPENTABLE: P0 none; P1 fixed (this commit); P2 (i) op-lock not-reproduced, (ii) fixed-run 2h expiry, (iii) general-QA lease/profile (needs operator). No policy/model/route/service changes this round; remote pointer / DO / live LINE = MANUAL_EXCLUDED (untested).
