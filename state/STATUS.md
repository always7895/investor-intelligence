...
- Acceptance (Mapika ACCEPT_SCOPED .8613): full Python 1569 tests 357.336s OK skip 3 (first 1568 run 1 schema failure retained); Worker 869 PASS 1 skip; typecheck/compile/PS5.1/7/4 gates PASS; focused 51/51 + gate 6/6. Bounded self-review NOT independent certification. Fail-closed residuals: inert fallback_sources; role/class/binding typo diagnostics; optional legacy descriptor compatibility intentional. Next lane: policy-descriptor referential consistency.
...
- `tests/test_source_registry.py`: +3 coverage-CLI subprocess regressions (fixture ledger equality, fixture-only ids, malformed/missing fail-closed, deterministic two runs; handle-owned `.tmp/` fixture, clean-checkout portable; no network). Verification: focused 17/17 OK + 4 gates + diffcheck.
