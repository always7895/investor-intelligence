...
- Fetched starting HEADf126f25d0fc3607396543560158335da784cab17, branch `fix/options-provenance-audit`/PR37. Source: `D:\Investor-Intelligence-LINE-Pi\_workspace\source`. PR39 isolated at d58fdaf in `_workspace/review-source-views`; baseline52e285f in `_workspace/review-baseline`. One writer, no implicit integration.
...
## Current audit: EXE model and manual THINK controls

- Candidate EXE now exposes none/minimal/low/medium/high/xhigh/max. none maps to thinking=false; other efforts request true. Saving model/mode uses the shared validated profile, preserves token/time bounds and sets qualification false. Busy operations disable both selectors. No model-family guesses, automatic best-mode or support claims.
- Native UI self-test opens the actual form with discovery/production actions disabled, clicks its real Use handler across two synthetic models/all modes, checks file/hash/unqualified state and actual PowerShell child snapshots for off/on. Invalid effort leaves valid profile bytes unchanged. Test profile/logs are isolated. Packager now executes profile/UI self-tests on its compiled EXE; no real package produced here.
- Focused native8 PASS (`thinking-ui-native.log`). Full683 initially failed only STATUS12003-byte budget (`thinking-ui-python-full.log`); trimmed historical repetition, gate unchanged. Final Python683/2 skipped, Worker227, typecheck, six gates and PS5.1/7 PASS (`thinking-ui-python-final.log`, `thinking-ui-worker.log`). New UI is not installed; real model mode support, automatic selection and completed user-facing inference remain unqualified.
- Prior profile-bound archive admission, actual ZIP/CLI negatives and Python682/Worker227/gates proofs remain in `git show f126f25:state/STATUS.md` and `archive-profile-*` logs. Q6-only legacy admission and rejection of unqualified thinking archives remain unchanged.
- Original local routing attempt failed before inference (`local-route-smoke-20260909T202648.json`); ad-hoc Int64 check failed, subsequent readonly diagnostics found max1/0 loaded models. Last runner query found0 idle reviewed runners. No model load, preset, installed-runtime, schedule, Production or real-LINE mutation. Non-FreeRelay discovery/auto-start and coordinated install acceptance remain open.
