...
- Fetched starting HEAD50dfa57bd95877d2e87e27f44f496aed636579b0, branch `fix/options-provenance-audit`/PR37. Source: `D:\Investor-Intelligence-LINE-Pi\_workspace\source`. PR39 isolated at d58fdaf in `_workspace/review-source-views`; baseline52e285f in `_workspace/review-baseline`. One writer, no implicit integration.
- Current-session authorization: repair project runner, isolated validation, then full installation/sealed publication/exact two-task restoration ONLY after all gates. No real LINE sends, broker operations, paid services or stale replay. This step: release-input/archive code/tests only; no Production mutation.
...
- HEAD-bound `state/r75-qa-live-current.ref.json` selects immutable receipt path/SHA via shared `r75_release_inputs.py`. No newest-file search or invalid-pointer fallback; regular committed files, bounded bytes and exact worktree match required. Byte-preserving Git attributes cover receipt checkout. Legacy defaults remain only when pointer truly absent.
- Validation/package use the same selector, model/freshness verifier and Windows path/SHA binding. New ZIP refs and mandatory adjacent QA receipt are checked independently against archive runtime bytes; never import archive code. Original receipts are not restamped. Selection alone does not qualify a release.
- Found destructive OutputRoot pre-clean and non-mandatory QA sidecar gaps. Existing output/source-child paths now reject; actual PS caller tests preserve sentinels. Synthetic rehashed ZIP/CLI negatives cover missing/tampered/stale QA, runtime drift and ref/Windows mismatch. Python697/2 skipped and Worker227/typecheck PASS; six gates/PS5.1/7 PASS, new Windows pending. Initial full run failed only STATUS12576-byte budget; trimmed, not relaxed.
- Separate read-only release audit: existing activation.ts and activation-v2.ts are outside hotfix allowlist; activation-v2 differs from protected536644d baseline by removal of stale options/universe carry-forward. Preserve that safety fix; versioned migration/recertification required, NOT hash/allowlist bypass. No full release/installation/publication/task restoration.
...
- Old receipts stay historical; current reference selects the new source-bound Q&A evidence above. Historical Q5 thinking=false/effort=none matrix passed10 cases844–1890ms; receipt `r75-qa-live-model-profile-qualification.json` starts2026-09-09T05:26:24.943379Z.24h/source/profile binding applies. Original xhigh failure remains. Do not restamp evidence or use another Router/model/preset/paid fallback.
...
1. Resolve protected activation drift through versioned migration/recertification; preserve no stale carry-forward. Full release validation remains blocked. Audit remaining producer/admission/time/source-binding logic. Complete genuine company/financial/order data and separate stock/options/macro report producers; preserve failures/conflicts and independent lineage. Fix actual thematic selection without changing scoring.
