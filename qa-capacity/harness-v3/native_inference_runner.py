...
def load_request_bytes(namespace):
            ...
            if content_length is not None:
                ...
                while True:
                    ...
                    try:
                        ...
                        break
                    if chunk_size == 0:
                        _interruptible_read(sock, 2, cancel_event, deadline, read_timeout_s)
                        break
                    while len(body) < len(body) + chunk_size:
                        chunk = _interruptible_read(sock, min(8192, chunk_size), cancel_event, deadline, read_timeout_s)
                        if not chunk:
                            break
                        body += chunk
                        if len(body) > body_bytes_max:
...
def run_live(namespace):
    ...
    try:
        ...
        raise WorkloadError("source_head_invalid")
    if len(source_head) != 40 or not all(c in "0123456789abcdef" for c in source_head):
        raise WorkloadError("source_head_invalid")
    ...
    request_bytes = load_request_bytes(namespace)
    ...
    evidence_out = {"request_sha256": req_digest, "binding_digest": binding_digest, "evidence": evidence}
...
def main(argv=None, namespace=None):
    ...
    try:
        ...
        if e.code in ("authority_missing", "authority_mismatch", "run_plan_mismatch"):
            return 2
        return 1
