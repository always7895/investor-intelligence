...
原卡片「完整文字版」全部送出 `Top20 文字`，實際只是同一七欄的文字格式。現在候選程式將它誠實改為「證據詳情」：指定公司、檢查卡片生成時間及資料取得新鮮度、列出來源與缺口。它不是完成版深度報告。生成時間核對也不是完整 snapshot SHA 綁定，後續仍需下列封存契約。
...
- 同次查詢的報告與pipeline時間戳必須固定使用同一快照讀取上下文，不能各自重新解析pointer而混輪。現有pointer為空值、無效結構或衝突身分時，應拒絕讀取，不可冒充「不存在」而退回直接鍵。現行Top20候選路由透過 `v213/public-snapshot.ts` 實作；受保留契約保護的 `storage.ts` 不改寫，其餘舊消費端仍需逐一遷移或重新認證。這不取代內容雜湊或卡片run/report-SHA完整綁定，後者仍待完成。
