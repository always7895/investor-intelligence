...
- 深度報告：完整資料表、推導、證據、反方資料、情境敏感度及缺漏。可以引用卡片事實，不能只是卡片文字重排，也不能為求篇幅加入空話。
- 兩者出自同一份驗證後研究資料；摘要與長報告分別儲存／生成。報告參照需包含 snapshot run ID、report ID、內容 SHA256 及標的身分。
...
- 股票、期權、宏觀三種詳報分別驗收。只有股票證據入口通過，不得宣稱全部深度分析完成。
