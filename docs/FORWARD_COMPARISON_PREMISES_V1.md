...
2. **C2 consumer contract:**
   ...
   - **C2b — option C IMPLEMENTED (see "C2b finding"):** `build_forward_comparison_artifact` in `scripts/v213_forward_comparison_render.py`; `data_report` stays unchanged; the forward comparison is emitted as its own digest-bound local JSON plus the C2a text, operator-facing only and `publication_eligible=false`. An archived-receipt contract (option B) is revisited together with C3 live qualification.
...
- **C. Separate local artifact (operator choice 2026-09-25):** keep `data_report` unchanged and emit the forward comparison as its own local JSON plus text, digest-bound, operator-facing only. Revisit B together with C3 live qualification.
