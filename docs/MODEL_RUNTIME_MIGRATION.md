...
Full CI run34382938201 on e540595 passed Windows validation, native EXE tests, immutable packaging, extracted Worker227 and isolated installation. The downloaded ZIP and three receipts plus QA evidence also passed the trusted-source verifier (SHA `13a9653b504486f790245d71693fc40302ebfa8018b47e322469a55de18741d4`). A second, downloaded isolated install matched all ten audited chain files and EXE bytes and passed native version/profile/THINK self-tests.

That additional install exposed a remaining inconsistency: packaged profile is the approved Q5 model, but legacy runtime-state `preferred_model` still says `RVN-Q6_K-multilingual-mtp`. This is not proof of an inference downgrade; the original receipt remains `release_qualified=false`. Current installer writers now emit `preferred_model=null`, `model_selection_authority=runtime_model_profile` and `model_profile_qualified=false`: installation does not select or certify a model. Repository consumers do not use these installation receipts to select models; the existing strict runtime profile remains authoritative. Native tests exercise all four installer callers without changing the operator profile, and packaging rejects metadata that selects a model or claims qualification (including non-primitive lookalikes). Fresh package/download acceptance is still required. No real installation, Production publication, task restoration or LINE test send was performed.
