...
That additional install exposed a remaining inconsistency: packaged profile is the approved Q5 model, but legacy runtime-state `preferred_model` still says `RVN-Q6_K-multilingual-mtp`. This is not proof of an inference downgrade; installer writers and consumers need correction/acceptance. The receipt therefore remains `release_qualified=false`. No real installation, Production publication, task restoration or LINE test send was performed.
