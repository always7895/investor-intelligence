...
## Doc-integrity correction of the options slice (2026-09-13, from `a01d202`)

Astra's single review of `options-admission.diff` accepted CODE24/typecheck and rejected the two doc changes. Wordings-only corrections, exact 2 docs (`state/STATUS.md`, this file); no source code, guard, test, runtime, env, credential, network, model, install, LINE or push change; no code-test rerun. (1) STATUS restored the exact full-history commands `git show b96167af:state/STATUS.md`, `git show 6ffe6c86877c220c28b67f712edb38ea125806ca:state/STATUS.md`, `git show 1ba1a97713f4b152ffcb3cae4950c17076c8d0d0:state/STATUS.md`, `git show 38860e7:state/STATUS.md` (the SHA-only short list was not equivalent). (2) STATUS verification now separates HISTORICAL SEC (68 post-F1 + 6/6 engine, recorded, not rerun) from the OPTIONS slice (24/24, `tsc --noEmit` 0, 3 changed doc files, 2 doc gates + security_check, recorded, not rerun) and states this correction reruns neither code-test group; the doc gates' full read scope is the 94-file Markdown inventory + fixed marker set, not merely the changed docs. (3) Both docs now qualify redirect refusal and `ProxyHandler({})` to the H6B collector's per-call urllib opener alone; non-SEC H6B UA generic; federation generic UA + `session.trust_env=False` (requests session, not urllib), redirect behavior unchanged and not qualified. Closed repetition in STATUS compressed into references to this audit doc; all 7 open findings, `publication_eligible=false` and README authority preserved. Receipts: `options-doc-integrity-*`, `options-final-receipt-v2.json`, `HANDOFF-options-admission-v2.md` (audit dir).
