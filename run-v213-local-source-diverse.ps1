...
function Test-Python([string]$Path){
    ...
    try{& $Path -c "import requests,sys; assert sys.version_info >= (3,10)" 2>$null;return$LASTEXITCODE-eq0}catch{return$false}
}
function Resolve-Python{
    ...
    $home=Join-Path $runtimeRoot 'python-3.12.10';$python=Join-Path $home 'python.exe'
    if(-not(Test-Path -LiteralPath $python -PathType Leaf)){
        & (Join-Path $ProjectRoot 'scripts\bootstrap_portable_python.ps1') -DestinationPath $home
        if($LASTEXITCODE-ne0){throw 'Verified portable Python bootstrap failed.'}
    }
    if(-not(Test-Python $python)){
        ...
        if($LASTEXITCODE-ne0){throw 'Hash-locked Python dependency installation failed.'}
        & $python -m pip check
        if($LASTEXITCODE-ne0){throw 'Python dependency check failed.'}
    }
...
try{
    ...
    $python=Resolve-Python;$env:PROJECT_PYTHON=$python;Load-SecContact
    ...
    if(-not$NoModelBridge){
        try{
            ...
            if($LASTEXITCODE-ne0){throw 'Source-diverse bridge returned nonzero.'}
            $bridgeReady=$true
    ...
    try{
        ...
        $args=@('scripts\v213_v21_progress_runner.py');if($Synthetic){$args+='--synthetic'}
        & $python @args
        if($LASTEXITCODE-ne0){throw 'Top20 engine failed.'}

        Stage 4 'Signed public snapshot build'
        & $python 'scripts\build_v21_public_snapshot.py'
        if($LASTEXITCODE-ne0){throw 'Public snapshot build failed.'}
        ...
        if($Synthetic){
            ...
            if($LASTEXITCODE-ne0){throw 'v2.1.2 five-field report failed.'}

            Stage 6 'Current-membership order evidence plus source-independence gate'
            & $python 'scripts\reconcile_v213_order_evidence.py'
            if($LASTEXITCODE-ne0){throw 'Order-evidence reconciliation failed.'}
            ...
            if($LASTEXITCODE-ne0){throw 'Multi-source independence gate failed; report promotion stopped.'}
            ...
            if($LASTEXITCODE-ne0){throw 'Seven-field report build failed.'}
            ...
            if($NoSync){
                ...
                if(Test-Path -LiteralPath $syncConfig -PathType Leaf){
                    ...
                    if($LASTEXITCODE-ne0){throw 'Signed public snapshot sync failed.'}
                    & .\sync-v212-top20-report.ps1 -ProjectRoot $ProjectRoot -LocalConfigPath $syncConfig
                    if($LASTEXITCODE-ne0){throw 'Signed five-field sync failed.'}
                }else{Write-Warning 'Signed-sync configuration is not installed; local outputs were retained.'}
                ...
                if(Test-Path -LiteralPath $activeConfig -PathType Leaf){
                    if(-not$NoAutoActivation-and$bridgeReady){
                        ...
                        if($LASTEXITCODE-ne0){throw 'Active source-diverse Worker/model route refresh failed.'}
                        ...
                        if($LASTEXITCODE-ne0){throw 'Seven-field sync failed.'}
