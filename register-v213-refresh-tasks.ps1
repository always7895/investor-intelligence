...
param(
    ...
    [switch]$ValidateOnly,
    [switch]$EnableSealedPublication
)
...
function Get-ActionArguments([string]$Slot){
    $arguments="-NoProfile -NonInteractive -ExecutionPolicy Bypass -File `"$runScript`" -RuntimeRoot `"$RuntimeRoot`" -Slot $Slot"
...
if($ValidateOnly){
    ...
    Write-Host "V213_R75_REFRESH_TASKS_VALIDATE = PASS; $MorningRefreshTime / $EveningRefreshTime; sealed_publication_enabled=$([bool]$EnableSealedPublication); production_mutation=false" -ForegroundColor Green
    exit 0
}
$backup=@{}
...
try{
    foreach($definition in $definitions){
        ...
        Register-ScheduledTask -TaskName $definition.Name -Action $action -Trigger $trigger -Settings $settings -Principal $principal -Description $description -Force|Out-Null
    }
    foreach($definition in $definitions){
        $task=Get-ScheduledTask -TaskName $definition.Name -ErrorAction Stop
        $action=@($task.Actions)|Select-Object -First 1
        if(-not$action-or[string]$action.Execute-ne$ps-or[string]$action.Arguments-notlike'*run-v213-scheduled-refresh.ps1*'-or[string]$action.Arguments-cne(Get-ActionArguments $definition.Slot)){
    ...
    foreach($name in $taskNames){
        try{if($backup[$name]){Register-ScheduledTask -TaskName $name -Xml ([string]$backup[$name]) -Force -ErrorAction Stop|Out-Null}else{if(Get-ScheduledTask -TaskName $name -ErrorAction SilentlyContinue){Unregister-ScheduledTask -TaskName $name -Confirm:$false -ErrorAction Stop}}}catch{$rollbackFailed=$true}
    }
    if($rollbackFailed){throw 'V213_TASK_UPGRADE_FAILED; rollback_unverified=true'}
    throw "V213 R75 task upgrade failed; rollback commands completed, independent readback still required. $failure"
}
[ordered]@{
    schema_version=2
    ...
    data_only=(-not[bool]$EnableSealedPublication)
    model_bridge_started_by_schedule=$false
    sealed_publication_enabled=[bool]$EnableSealedPublication
    production_mutation_by_schedule=[bool]$EnableSealedPublication
    wake_to_run=$true
    restart_count=3
