...
$settings=New-ScheduledTaskSettingsSet -StartWhenAvailable -WakeToRun -MultipleInstances IgnoreNew -ExecutionTimeLimit (New-TimeSpan -Minutes 100) -RestartCount 3 -RestartInterval (New-TimeSpan -Minutes 10)
...
try{
    ...
    foreach($definition in $definitions){
        ...
        if(-not$settingsObserved.StartWhenAvailable-or-not$settingsObserved.WakeToRun-or[int]$settingsObserved.RestartCount-lt3){throw "Scheduled task reliability settings failed: $($definition.Name)"}
    }
}catch{
