...
$logPath = Join-Path $logRoot ('serenity-latest-' + (Get-Date -Format 'yyyyMMdd-HHmmss') + '.log')
Start-Transcript -Path $logPath -Force | Out-Null
$oldUnbuffered = $env:PYTHONUNBUFFERED
$env:PYTHONUNBUFFERED = '1'

function Stage([int]$Number, [string]$Name) { Write-Host ("II_STAGE {0}/8 | {1}" -f $Number, $Name) -ForegroundColor Cyan }
...
function Resolve-Python {
    ...
    if (-not (Test-Path -LiteralPath $python -PathType Leaf)) {
        ...
        if ($LASTEXITCODE -ne 0) { throw 'Verified portable Python bootstrap failed.' }
    }
    ...
    if (-not (Test-Python $python)) { throw 'Verified Python runtime is unavailable.' }
    return $python
}
...
try {
    if ($SelfTest) {
        ...
        foreach ($command in @(
            ...
            & $python @command
            if ($LASTEXITCODE -ne 0) { throw "Self-test failed: $($command -join ' ')" }
        }
        & (Join-Path $ProjectRoot 'sync-v213-activation-bundle.ps1') -SelfTest
        if ($LASTEXITCODE -ne 0) { throw 'Activation-bundle client self-test failed.' }
        Write-Host "V213_SERENITY_LATEST_REFRESH_ENTRYPOINT_SELF_TEST = PASS; python=$python; per_ticker_multisource=true; latest_available=true" -ForegroundColor Green
        return
    }
    ...
    if (-not $NoModelBridge) {
        ...
        catch { Write-Warning ("Exact-model bridge unavailable; public-data research continues, but formal activation remains blocked. " + $_.Exception.Message) }
    }
    ...
    try {
        ...
        if ($Synthetic) { $engineArgs += '--synthetic' }
        & $python @engineArgs
        if ($LASTEXITCODE -ne 0) { throw 'Top20 provisional candidate engine failed.' }
        ...
        if ($Synthetic) {
            ...
            if ($LASTEXITCODE -ne 0) { throw 'Synthetic boundary contract failed.' }
            Write-Host 'II_STAGE 5-8/8 | synthetic mode intentionally skips live source and sync stages' -ForegroundColor DarkGray
        }
        else {
            ...
            if ($LASTEXITCODE -ne 0) { throw 'Order-evidence reconciliation failed.' }
            & $python 'scripts\build_v213_scheduled_top20_report.py' '--baseline' 'data\cache\v213_order_evidence_runtime.json'
            if ($LASTEXITCODE -ne 0) { throw 'Seven-field draft build failed.' }

            Stage 6 'Federation, source independence, latest evidence and Serenity factor audit'
            & $python 'scripts\v213_source_federation.py'
            if ($LASTEXITCODE -ne 0) { throw 'Live source federation failed.' }
            & $python 'scripts\v213_source_federation_gate.py'
            if ($LASTEXITCODE -ne 0) { throw 'Live source federation gate failed.' }
            ...
            if ($LASTEXITCODE -ne 0) { throw 'TAM-capture claim guard failed.' }
            & $python 'scripts\v213_serenity_latest_multisource_audit.py' '--normalize'
            if ($LASTEXITCODE -ne 0) { throw 'Final Serenity factor ledger audit failed.' }
            Write-Host 'II_PROGRESS latest per-ticker source, recency, factor and thesis-killer gates PASS' -ForegroundColor Green
            ...
            if ($NoSync) {
                ...
                elseif ($bridgeReady) {
                    ...
                    if ($LASTEXITCODE -ne 0) { throw 'Active source-diverse Worker/model route refresh failed.' }
                    Write-Host 'V213_SERENITY_LATEST_SCHEDULE_MODEL_AND_BUNDLE_REFRESH = PASS' -ForegroundColor Green
                }
                else {
        ...
        Write-Host 'INVESTOR_INTELLIGENCE_V213_SERENITY_LATEST_MULTISOURCE = PASS' -ForegroundColor Green
    ...
    Write-Error ("V213_SERENITY_LATEST_REFRESH_FAILED: " + $_.Exception.Message)
