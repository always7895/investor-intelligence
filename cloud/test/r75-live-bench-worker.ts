  ...
  async fetch(request: Request, env: any, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/setup" && request.method === "POST") {
      ...
      await env.PUBLIC_CACHE.put("v213:top20-report:latest", JSON.stringify({
        ...
        records: Array.from({length:20},(_,i)=>({schema_version:2,rank:i+1,ticker:`T${String(i).padStart(2,"0")}`,name:`T${String(i).padStart(2,"0")} 公司`,long_term_return_pct:null,short_term_return_pct:null,industry:"合成測試",profit_summary:"未提供測試數值",current_orders:V213_NO_CURRENT_ORDERS,future_orders_estimate:V213_NO_FUTURE_ORDER_ESTIMATE,long_term_window:"2y_cagr",short_term_window:"6m_price_return",market_source:"yfinance",profit_source:"sec_edgar",orders_as_of:stamp,orders_confidence:"UNAVAILABLE",current_order_source_urls:[],future_order_source_urls:[],numeric_total_order_estimate_prohibited:true,retrieved_at:stamp,provider_scope:"public_only",owner_watchlist_inherited:false}))
      }));
