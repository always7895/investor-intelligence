...
import { v21Top20Answer } from "./top20";

export interface V21Env extends QaEnv, LineEnv, V21AdminEnv, V21BroadcastEnv {
...
async function pairingCodeValid(env: V21Env, code: string): Promise<boolean> {
  ...
  for (const event of events) {
    ctx.waitUntil(
      processLineEvent(env, ctx, event).catch((error) => {
        console.error("V21_LINE_EVENT_FAILED", error instanceof Error ? error.name : "UNKNOWN");
      }),
    );
  }
...
async function handleAdmin(
  ...
  try {
    ...
    if (pathname === "/v21/admin/public-snapshot") {
      return jsonResponse({ status: "accepted", ...(await ingestV21PublicSnapshot(body, env)) });
    }
    if (pathname === "/v21/admin/status") {
      return jsonResponse({
        ok: true,
        owner_paired: await ownerPairingStatus(env),
        public_snapshot_available: Boolean(await env.PUBLIC_CACHE.get("snapshot:current")),
      });
    }
    ...
    return jsonResponse(
      { ok: false, code: error instanceof Error ? error.message : "V21_ADMIN_FAILED" },
      401,
    );
  }
}
...
function health(): Response {
  return jsonResponse({
    ok: true,
    service: "investor-intelligence-v21-owner-line",
    owner_only: true,
  ...
  async scheduled(
    ...
    ctx.waitUntil(
      scheduledV21Broadcast(env, controller.cron, controller.scheduledTime)
        .then((result) => console.log("V21_SCHEDULED_TOP20", String(result.status ?? "unknown")))
        .catch((error) => console.error("V21_SCHEDULED_TOP20_FAILED", error instanceof Error ? error.name : "UNKNOWN")),
    );
  },
