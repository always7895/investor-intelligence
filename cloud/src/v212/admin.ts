...
import { parseV212Top20Report } from "./top20-report";

export async function ingestV212Top20Report(
  body: string,
  env: V21AdminEnv,
): Promise<{ run_id: string; report_count: number }> {
  let raw: unknown;
  try {
    raw = JSON.parse(body);
  } catch {
    throw new Error("V212_TOP20_REPORT_JSON_INVALID");
  }
  const report = parseV212Top20Report(raw);
  if (!report) throw new Error("V212_TOP20_REPORT_SCHEMA_INVALID");
  ...
  if (expected.some((ticker, index) => ticker !== actual[index])) {
    throw new Error("V212_TOP20_REPORT_ORDER_MISMATCH");
  }

  await env.PUBLIC_CACHE.put(
    `snapshot:${runId}:v212:top20-report:latest`,
    JSON.stringify(report),
    { expirationTtl: 259200 },
  );
  return { run_id: runId, report_count: report.records.length };
}
