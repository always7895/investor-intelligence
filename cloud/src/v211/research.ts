  ...
  if (answer === "OPTION_DATA_UNAVAILABLE") {
    if (query.ticker) {
      ...
      if (ticker === "TSM" || ticker === "2330.TW") {
        return [
          "📈【台積電（TSM / 2330.TW）期權與對沖操作指引】",
          "🎯 標的：台積電（美股 ADR: TSM / 台灣期交所: CDO 台積電期貨）",
          "──────────────────────────────",
          "💡 交易市場與路徑確認：",
          "• 美股 ADR 每週期權：在 IBKR 搜尋「TSM」，進入紐約證交所（NYSE）每週期權鏈，流動性為全球半導體之冠！",
          "• 台灣期貨交易所：搜尋「台積電」或「CDO（台積電股票期貨）」，每口表彰 2,000 股現貨標的。",
          "",
          "🛡️【不賣股為第一優先】Covered Call 賣買權收租建議：",
          "• 履約價挑選（Strike K）：全球先進製程與先進封裝絕對壟斷，長線抱緊，絕不賣股！建議挑選價外 +15%～+25%（Delta 0.12～0.18）之每週合約，享有極厚安全墊。",
          "• 到期天數（DTE）：5～14 天每週期權，週週穩健收取時間價值（Theta）。",
          "• 掛單心法：務必採用「限價單（Limit Order）」在推薦限價區間內成交，切勿使用市價單避免被吃滑點！",
          "",
          "🟡 Cash-Secured Put 賣賣權折價低接建議：",
          "• 履約價挑選：選擇自願長線建倉之強支撐價位（現價折價 10%～20%），預留 100% 現金保證金，折價接盤全球半導體總閥門。",
        ].join("\n");
      }
      if (ticker === "IQE" || ticker === "IQE.L") {
        return [
          "📈【IQE plc 期權交易與 IBKR 操作指引】",
          "🎯 標的：IQE plc（英國倫敦 LSE / ICE 期權 / 幣別: GBP 英鎊）",
          "──────────────────────────────",
          "💡 IBKR（盈透證券）交易路徑確認：",
          "• 在 TWS 或 IBKR Mobile 搜尋「IQE」，選擇「IQE PLC (LSE - Stocks/Options)」，即可連通英國倫敦期權市場！",
          "",
          "🛡️【不賣股為第一優先】Covered Call 賣買權收租建議：",
          "• 履約價挑選：化合物半導體與量子點雷射磊晶長線爆發潛力極大，絕不賣股！建議挑選價外 +20%～+30% 之最高可用 Strike（Delta < 0.15）。",
          "• 建議合約天數：30～60 天月選合約，賺取高額時間價值（Theta）。",
          "",
          "🟡 Cash-Secured Put 賣賣權折價低接建議：",
          "• 履約價挑選：選擇自願長線建倉之強支撐價位（現價折價 15%～25%），收取權利金降低實質持股成本，預留 100% 現金保證金。",
        ].join("\n");
      }
