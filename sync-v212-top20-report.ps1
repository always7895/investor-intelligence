...
if ([string]::IsNullOrWhiteSpace($ReportPath)) {
    $ReportPath = Join-Path $ProjectRoot 'data\cache\v212_top20_report_public_latest.json'
}
...
if (-not (Test-Path -LiteralPath $ReportPath -PathType Leaf)) {
    throw 'The v2.1.2 five-field Top 20 report does not exist.'
}
...
$endpoint = [uri]($baseEndpoint.GetLeftPart([UriPartial]::Authority) + '/v212/admin/top20-report')
...
try {
    ...
    if ([string]$response.status -ne 'accepted' -or [int]$response.report_count -ne 20) {
        throw 'The v2.1.2 Worker did not accept the five-field Top 20 report.'
    }
    Write-Host "V2.1.2 five-field Top 20 report promoted: $($response.run_id)" -ForegroundColor Green
}
finally {
