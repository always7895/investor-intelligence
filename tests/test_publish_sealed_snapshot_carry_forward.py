...
class LkgHelperTests(unittest.TestCase):
    ...
    def test_due_promote_and_backoff(self):
        ...
        self.assertEqual(carry.refresh_due(now + timedelta(hours=1, minutes=59), self.lkg)["reason"], "BACKOFF_ACTIVE")
        self.assertTrue(carry.refresh_due(now + timedelta(hours=2, minutes=1), self.lkg)["due"])
