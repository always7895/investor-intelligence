...
try {
    ...
    try {
        $engineArguments = @('scripts\v211_serenity_top20.py')
        if ($Synthetic) { $engineArguments += '--synthetic' }
        & $PythonExe @engineArguments
        if ($LASTEXITCODE -ne 0) {
            throw "v$PatchVersion Serenity research-universe pipeline failed with exit code $LASTEXITCODE."
        }
        ...
        if (-not $Synthetic) {
            ...
            # Synthetic acceptance uses the snapshot builder's own option fixture.
